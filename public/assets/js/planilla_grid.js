$(document).ready(function () {
  ...
  $("#btn-agregar-trabajador").on("click", function () {
    ...
    if (!selectedOption.val()) {
      Swal.fire(
        "Error",
        "Debes seleccionar un trabajador para agregar.",
        "error"
      );
      return;
    }
    ...
    const sueldoBase = selectedOption.data("sueldo-base") || 0;
    const esPartTime = selectedOption.data("es-part-time") || 0;

    if (
      $gridBody.find(`.fila-trabajador[data-id-trabajador="${id}"]`).length > 0
    ) {
      Swal.fire(
        "Error",
        `El trabajador ${nombre} ya está en la grilla.`,
        "warning"
      );
      return;
    }
    ...
    $nuevaFila
      .removeAttr("id")
      .addClass("fila-trabajador")
      .attr("data-id-trabajador", id);
    $nuevaFila.attr("data-estado-previsional", estado);
    $nuevaFila.attr("data-sueldo-base", sueldoBase); // Guardamos base

    let htmlFila = $nuevaFila
      .html()
      .replace(/{ID}/g, id)
      .replace(/{NOMBRE}/g, nombre)
      .replace(/{ES_PART_TIME}/g, esPartTime)
      .replace(/{RUT}/g, rut);

    $nuevaFila.html(htmlFila);

    // Asignar sueldo base inicial
    $nuevaFila.find('input[name="sueldo_imponible[]"]').val(sueldoBase);
  ...
  // Validar Días Trabajados y Calcular Proporcional
  $gridBody.on("input", ".dias-trabajados", function () {
    ...
    const $fila = $(this).closest("tr");
    const sueldoBase = parseFloat($fila.attr("data-sueldo-base")) || 0;

    // Solo calcular si hay sueldo base y días válidos (y si no se ha modificado manualmente el sueldo,
    // pero el requerimiento dice: "el cambio en Días manda". Así que siempre sobrescribimos si se cambian días).
    if (sueldoBase > 0) {
      const nuevoSueldo = Math.round((sueldoBase / 30) * dias);
      $fila.find('input[name="sueldo_imponible[]"]').val(nuevoSueldo);
    }
  });
  ...
  $gridBody.on("change", ".col-cotiza-cesantia .form-check-input", function () {
    ...
    const $hiddenInput = $checkbox
      .closest("td")
      .find('input[name="cotiza_cesantia_pensionado_hidden[]"]');
  ...
  $("#btn-guardar-planilla").on("click", function () {
    ...
    $gridBody.find(".fila-trabajador").each(function () {
      ...
      const cotizaCesantiaValor =
        $fila.find(".cotiza-cesantia-hidden").val() || "0";

      // Construir el objeto para este trabajador
      let trabajador = {
        trabajador_id: $fila.find('input[name="trabajador_id[]"]').val(),
        es_part_time: $fila.find('.es-part-time-input').val() || 0,
        sueldo_imponible:
          $fila.find('input[name="sueldo_imponible[]"]').val() || 0,
        bonos_imponibles:
          $fila.find('input[name="bonos_imponibles[]"]').val() || 0,
        dias_trabajados:
          $fila.find('input[name="dias_trabajados[]"]').val() || 30,
        ...
        adicional_salud_apv:
          $fila.find('input[name="adicional_salud_apv[]"]').val() || 0,
        cesantia_licencia_medica:
          $fila.find('input[name="cesantia_licencia_medica[]"]').val() || 0,
    ...
    fetch(GUARDAR_PLANILLA_URL, {
      ...
      .then((data) => {
        if (data.success) {
          ...
          Swal.fire(
            "Error",
            data.message || "No se pudo guardar la planilla.",
            "error"
          );
        }
      })
      .catch((error) => {
        Swal.fire(
          "Error",
          "Hubo un problema de conexión: " + error.message,
          "error"
        );
      });
  });
